...
export default {
   ...
   */
  updateTotalPomodoros ({commit, state}, totalPomodoros) {
    if (!totalPomodoros) {
      return
    }
    totalPomodoros = parseInt(totalPomodoros)
    if (state.statisticsRef) {
      state.statisticsRef.update({totalPomodoros})
    } else {
      commit('updateTotalPomodoros', totalPomodoros)
    }
  },
  /**
